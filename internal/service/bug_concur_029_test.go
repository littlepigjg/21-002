...
func buildCase(base string, n int, tokensPerSen int) []model.Sentence {
	tokens := []string{"背景", "方案", "设计", "实现", "细节", "测试", "验证", "总结", "展望", "部署", "架构", "性能", "优化", "安全", "监控"}
	out := make([]model.Sentence, 0, n)
	for i := 0; i < n; i++ {
...
func sentenceSubstrings(sentences []model.Sentence) []string {
...
func TestBugConcur029_SummaryOrderAndRace(t *testing.T) {
	...
	for outer := 0; outer < 6; outer++ {
		...
		summarizer := NewSummarizeService(4)
