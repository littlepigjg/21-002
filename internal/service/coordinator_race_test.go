...
func buildServicesForRaceTest() (*ArticleService, *HistoryService, *TaskService) {
	...
	coord := NewAnalysisCoordinator(2048)
	articleSvc := NewArticleService(mem, mem, analyzer, ids, coord, 100000)
	historySvc := NewHistoryService(mem, mem, coord)
	taskSvc := NewTaskService(mem, mem, mem, analyzer, ids, nil, coord, 100000)
