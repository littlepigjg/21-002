...
func NewTaskService(tasks store.TaskStore, articles store.ArticleStore, results store.ResultStore, analyzer *Analyzer, ids *store.IDGenerator, queue *taskqueue.Queue, maxLen int, rc *cache.ResultCache) *TaskService {
	if maxLen <= 0 {
		maxLen = 100000
	}
	if rc == nil {
