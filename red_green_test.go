...
func buildTestServices() (*service.ArticleService, *service.TaskService, *store.MemoryStore) {
	st := store.NewMemoryStore()
	ids := store.NewIDGenerator()
	stopwords := textutil.NewStopwordSet()
	pre := service.NewPreprocessor(stopwords)
	tfidf := service.NewTfidfService(20)
	textrank := service.NewTextRankService(20, 0.85)
	summ := service.NewSummarizeService(3)
	analyzer := service.NewAnalyzer(pre, tfidf, textrank, summ)
	queue := taskqueue.NewQueue(1024)
	articleSvc := service.NewArticleService(st, st, analyzer, ids, 1000000)
	taskSvc := service.NewTaskService(st, st, st, analyzer, ids, queue, 1000000)
